...
import { SiteAuditSummary, AuditIssue } from "../../../types/audit";
import {
  Sparkles,
  ArrowRight,
  RefreshCw,
...
export default function AuditsPage() {
  const [audits, setAudits] = React.useState<SiteAuditSummary[]>([]);
  ...
  const handleRunScan = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setScanMessage(data.message || "Audit scan completed.");
        await fetchAudits();
      }
    } catch (err) {
      console.error(err);
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-5">
        <Card className="border-primary/40 bg-card">
          <CardContent className="p-5 text-center">
            ...
            <div className="text-4xl font-black text-emerald-400 mt-2">{primaryAudit.overallScore}/100</div>
            <span className="text-[10px] text-emerald-400 font-semibold mt-1 block">PostgreSQL Synced</span>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="p-5 text-center">
            ...
            <div className="text-4xl font-black text-foreground mt-2">{primaryAudit.seoScore}/100</div>
        ...
        <Card>
          <CardContent className="p-5 text-center">
            ...
            <div className="text-4xl font-black text-foreground mt-2">{primaryAudit.contentScore}/100</div>
        ...
        <Card>
          <CardContent className="p-5 text-center">
            ...
            <div className="text-4xl font-black text-red-400 mt-2">{primaryAudit.criticalIssuesCount}</div>
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between pb-4 border-b border-border/60">
          ...
          <Badge variant="outline" className="text-xs">
            {primaryAudit.issues ? primaryAudit.issues.length : 0} Issues Found
          </Badge>
        </CardHeader>
        <CardContent className="p-0 divide-y divide-border/60">
          {!primaryAudit.issues || primaryAudit.issues.length === 0 ? (
            <div className="p-8 text-center text-xs text-muted-foreground">
              Zero issues detected. Your WordPress sites pass all diagnostic contract tests!
            </div>
          ) : (
            primaryAudit.issues.map((issue: AuditIssue) => (
